...
// 合并css文件
gulp.task('css_concat', ['replace_css_um'], () => {
    return gulp.src(cssArray)
        .pipe(concat('xk.css'))
        .pipe(gulp.dest('src/css'))
});
// 压缩css文件、为css文件添加上版本号
gulp.task('css_min_version', ['css_concat'], () => {
...
// 合并js文件
gulp.task('js_concat_xk', ['copy_js_umeditor', 'copy_js_date', 'replace_js_login'], () => {
    return gulp.src(xk_jsArray)
        .pipe(concat('xk.js'))
        .pipe(gulp.dest('src/js'));
});
gulp.task('js_concat_websocket', () => {
    return gulp.src(websocket_jsArray)
        .pipe(concat('websocket.js'))
        .pipe(gulp.dest('src/js'));
});
// 压缩js文件，为js文件添加上版本号
gulp.task('js_min_version', ['js_concat_xk', 'js_concat_websocket'], () => {
...
// html模板更改引用路径
gulp.task('html_replace', ['css_min_version', 'js_min_version', 'image_version'], () => {
