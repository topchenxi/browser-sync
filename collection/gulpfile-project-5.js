...
// 复制静态文件
gulp.task('copyStaticFile', ['copyFonts'], () => {
...
gulp.task('usemin', ['copyStaticFile', 'md5-all'], () => {
...
gulp.task('uglifyJs', ['jshint', 'usemin'], () => {
    return gulp.src([
            paths.output + '/js/*.js'
        ]).pipe(uglify({
            mangle: false
...
gulp.task('rename-index', ['clean', 'uglifyJs'], () => {
...
gulp.task('md5-all', () => {
    ...
    let jsFilter = filter(files.js, { restore: true }),
...
gulp.task('renameDir', ['rename-index'], () => {
    rimraf.sync('./build');
    // 文件拷贝
